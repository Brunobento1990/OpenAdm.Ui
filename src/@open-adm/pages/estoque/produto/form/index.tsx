...
export function ProdutoForm(props: IFormTypes) {
  ...
  const [itensTabelaDePreco, setItensTabelaDePreco] = useState<IItensTabelaDePreco[]>([])
  const Tab = useTabApp()
  const form = useFormikAdapter<IProduto>({
    ...
    validationSchema: new YupAdapter().string('descricao').string('categoriaId').build(),
    onSubmit: submit
  })
  ...
  async function submit() {
    const body = {
      ...form.values,
      novaFoto: form.values.novaFoto ? recortarBase64(form.values.novaFoto).base64 : undefined,
      tamanhosIds: form.values.tamanhos?.map(x => x.id),
      pesosIds: form.values.pesos?.map(x => x.id),
      tabelaDePrecoId: tabelaDePreco?.id,
      itensTabelaDePreco
    }
    const response = props.action === 'create' ? await create.fetch(body) : await update.fetch(body)
    if (response) {
  ...
  async function init() {
    const responseTabelaDePreco = await obterTabelaDePrecoAtivaPorProdutoId.fetch(id as string)
  ...
  const loading = create.status === 'loading' || update.status === 'loading' || obter.status === 'loading'
  ...
  return (
    <FormRoot.Form
      ...
      footer={{
        children: (
          <BoxApp display='flex' alignItems='center' justifyContent='end' width='100%' paddingLeft='1rem'>
            <CheckBoxApp
              id='vendaSomenteComEstoqueDisponivel'
              label='Venda com estoque'
              value={form.values.vendaSomenteComEstoqueDisponivel}
              onChange={form.onChange}
            />
            {props.action === 'edit' && <ModalEstoque produtoId={form.values.id} />}
          </BoxApp>
        )
      }}
    >
      ...
      <CollapseApp in={Tab.value === 1}>
        {!tabelaDePreco ? (
          <BoxApp>
            <TextApp texto='Não há uma tabela de preço ativa' color='primary' />
          </BoxApp>
        ) : (
          <>
            <BoxApp marginTop='1rem' display='flex' alignItems='center' gap='1rem'>
              ...
              <>
                ...
                {form.values.pesos.map((peso, index) => {
                  const itemTabelaDePreco = itensTabelaDePreco.find(
                    x => x.pesoId === peso.id && x.produtoId === form.values.id
                  )
                  return (
                    <FormRoot.FormRow key={peso.id} spacing={3}>
                      ...
                      <FormItemRow sm={3} xs={12}>
                        <InputApp
                          ...
                          mask={MaskType.MONEY}
                          id={`${peso.id}-${index}`}
                          onChange={(_, value) => onChangePrecoPeso(peso, value, 'valorUnitarioAtacado')}
                        />
                      </FormItemRow>
                      <FormItemRow sm={3} xs={12}>
                        <InputApp
                          ...
                          mask={MaskType.MONEY}
                          id={`${peso.id}-${index}`}
                          onChange={(_, value) => onChangePrecoPeso(peso, value, 'valorUnitarioVarejo')}
                        />
                      </FormItemRow>
            ...
            {form.values.tamanhos?.length > 0 && (
              <>
                ...
                {form.values.tamanhos.map((tamanho, index) => {
                  const itemTabelaDePreco = itensTabelaDePreco.find(
                    x => x.tamanhoId === tamanho.id && x.produtoId === form.values.id
                  )
                  return (
                    <FormRoot.FormRow key={tamanho.id} spacing={3}>
                      <FormItemRow sm={3} xs={12}>
                        <InputApp readonly value={tamanho.descricao} label={'Tamanho'} />
                      </FormItemRow>
                      <FormItemRow sm={3} xs={12}>
                        <InputApp
                          value={itemTabelaDePreco?.valorUnitarioAtacado}
                          ...
                          id={`${tamanho.id}-${index}`}
                          readonly={readonly}
                          onChange={(_, value) => onChangePrecoTamanho(tamanho, value, 'valorUnitarioAtacado')}
                        />
                      </FormItemRow>
                      <FormItemRow sm={3} xs={12}>
                        <InputApp
                          ...
                          mask={MaskType.MONEY}
                          id={`${tamanho.id}-${index}`}
                          onChange={(_, value) => onChangePrecoTamanho(tamanho, value, 'valorUnitarioVarejo')}
                        />
                      </FormItemRow>
                      <DividerApp width='100%' />
                    </FormRoot.FormRow>
