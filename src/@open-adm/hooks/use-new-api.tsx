...
export function useNewApi(props: propsUseApi) {
  ...
  const [statusRequisicao, setStatusRequisicao] = useState<StatusRequisicao | undefined>(props.statusInicial)
  ...
  function erro(error: any) {
    if (error?.code === 'ERR_NETWORK') {
      snack.show(`Erro de conexão com nossos servidores, tente novamente, ou entre em contato com o suporte.`, 'error')
      return
    }
  ...
  async function action<T = unknown>(propsFecth?: propsFecth): Promise<T | undefined> {
    try {
      if (abortControllerRef.current) {
        abortControllerRef.current.abort()
      }
      ...
      const response = await api.request({
        url: propsFecth?.urlParams ? `${props.url}${propsFecth?.urlParams}` : props.url,
