...
interface propsTabelaComDrag {
  ...
  width?: number | string
  marginTop?: string
  borderCollapse?: 'collapse' | 'inherit' | 'initial' | 'revert' | 'revert-layer' | 'separate' | 'unset'
  loading?: boolean
  rowHeight?: number
...
export function TabelaComDrag(props: propsTabelaComDrag) {
  ...
  const columns = useMemo(() => {
    ...
    if (parsedColunas && Array.isArray(parsedColunas) && parsedColunas.length > 0) {
      return [...mergeColumns(parsedColunas, props.columns).map(addHeaderTooltip)]
    }
  ...
  return (
    ...
    >
      ...
      <AgGridReact
        ...
        noRowsOverlayComponent={() => <TextApp texto='Não há registros' />}
        loadingOverlayComponent={() => <LoadingAppTexto comBox />}
        suppressDragLeaveHidesColumns
        suppressScrollOnNewData={props.suppressScrollOnNewData}
        getRowId={params => String(params.data.id || params.data.key || JSON.stringify(params.data))}
        onColumnMoved={async event => {
          if (!event.finished || !props.nomeDaTabela) return
          const orderedDefs = event.api.getAllGridColumns().map(col => col.getColDef())
...
export function mergeColumns(backendCols: any[], originalCols: TypeColumns[]): TypeColumns[] {
  return originalCols.map(backendCol => {
    const original = backendCols.find(col => col.field === backendCol.field)
    return {
      ...backendCol,
