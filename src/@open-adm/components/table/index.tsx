  ...
  async function init() {
    try {
      ...
      const response = await get<any>(
        `${props.url}?skip=${page - 1}${statusPedidoFiltro !== '0' && props.isPedido ? `&statusPedido=${parseInt(statusPedidoFiltro) - 1}` : ''}&search=${getValues('search')}`
      )
      if (response?.values) {
            ...
            <>
              ...
              {props.view && props.routeView && (
                <Tooltip title='Visualizar' placement='top'>
                  <IconButton
                    onClick={() =>
                      router.push(`${props.routeView}/${params.data.id}`).catch(err => {
              ...
              {props.edit && props.routeEdit && (
                <Tooltip title='Editar' placement='top'>
                  <IconButton
                    onClick={() =>
                      router.push(`${props.routeEdit}/${params.data.id}`).catch(err => {
                        if (!err.cancelled) throw err
                      })
                    }
                  >
    ...
    <>
      <Card>
        ...
        <form noValidate autoComplete='off' onSubmit={handleSubmit(onSubmit)}>
          <Grid container size={{ xs: 12 }} spacing={2} justifyContent='center' sx={{ marginBottom: 5 }}>
            <Grid size={{ xs: 7, md: 9 }}>
              <Controller
                name='search'
                control={control}
                ...
                render={({ field: { value, onChange, onBlur } }) => (
                  <CustomTextField fullWidth label='Pesquisa' value={value} onBlur={onBlur} onChange={onChange} />
                )}
              />
            </Grid>
            <Grid size={{ xs: 4, md: 2 }} marginTop={5}>
        ...
        {props.add && props.routeAdd && (
          <Box width='90%' display='flex' alignItems='center' justifyContent='end' marginBottom={5}>
            <Button
              onClick={() =>
                router.push(props.routeAdd ?? '').catch(err => {
                  if (!err.cancelled) throw err
                })
              }
              variant='contained'
              ...
              endIcon={<IconifyIcon icon='gala:add' />}
            >
              Novo
            </Button>
          </Box>
          ...
          <Grid size={{ xs: 12 }} sx={{ padding: 5 }}>
            <FormControl>
              ...
              <RadioGroup
                ...
                name='status'
                row
                value={statusPedidoFiltro}
                onChange={e => setStatusPedidoFiltro(e.target.value)}
              >
        ...
        <TabelaComDrag rows={rows} columns={[...defaultColuns(), ...props.columns, ...optionsColumns()]} />
      </Card>
      <Card
        sx={{
          marginTop: 5,
